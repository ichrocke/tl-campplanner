...
$pdo = getDB();
cleanupExpiredRooms();
$stmt = $pdo->prepare('SELECT state_json, version, IFNULL(locked, 0) as locked, IFNULL(TIMESTAMPDIFF(SECOND, NOW(), expires_at), -1) as expires_in FROM rooms WHERE id = ?');
$stmt->execute([$roomId]);
...
// expires_in: -1 = unbegrenzt, 0+ = Sekunden bis Ablauf
$expiresIn = intval($room['expires_in']);

if ($since > 0 && intval($room['version']) <= $since) {
    jsonResponse(['changed' => false, 'version' => intval($room['version']), 'locked' => $locked, 'expiresIn' => $expiresIn]);
}
